...
//     // Log để kiểm tra giá trị orderId
//     try {
...
const Order = () => {
  ...
  // Update order status to 'Huỷ'
  const handleCancelOrder = async (orderId) => {
    if (!window.confirm("Bạn có chắc chắn muốn huỷ đơn hàng này không?")) return;
    try {
      await orderAPI.updateOrderStatus(orderId, { status: 'Huỷ' }); // Update status to 'Huỷ'
      setOrders(orders.filter((order) => order.id !== orderId)); // Refresh orders list
      alert("Huỷ đơn hàng thành công!");
    } catch (error) {
      console.error("Error cancelling order:", error);
      alert("Không thể huỷ đơn hàng.");
  ...
  const handleDeleteOrder = async (orderId) => {
    ...
    try {
      await orderAPI.deleteOrder(orderId); // Call API to delete order
      setOrders(orders.filter((order) => order.id !== orderId)); // Refresh orders list
